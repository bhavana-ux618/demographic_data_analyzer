...
def calculate_demographic_data(print_data=True):
    ...
    higher_education_rich = round(
        (
            len(higher_education[higher_education["income"] == ">50K"])
    ...
    lower_education_rich = round(
        (
            len(lower_education[lower_education["income"] == ">50K"])
            / len(lower_education)
        )
        * 100,
        1,
    ...
    rich_percentage = round(
        (
            len(num_min_workers[num_min_workers["income"] == ">50K"])
            / len(num_min_workers)
        )
        * 100,
        1,
    ...
    country_rich = df.groupby("native-country")["income"].apply(
        lambda x: (x == ">50K").sum()
    )

    country_total = df.groupby("native-country").size()

    country_percentages = (country_rich / country_total) * 100
